	...
	def read_rounds(self, start, end):
		...
		while keep_reading:
			...
			if len(ln) < 1:
				...
				log_entry.total_time               = int(entries[7])
				log_entry.usage_mean_per           = int(entries[8])
				log_entry.usage_stan_per           = int(entries[9])
				log_entry.usage_CPU_mean           = int(entries[10])
				log_entry.usage_mem_mean           = int(entries[11])
